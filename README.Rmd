...
The function `get_output` takes a fitted `mgcv::gam` model and returns computed estimates and standard errors for any random effects and/or Markov random field spatial smoothing components, attached to an appropriate (spatial) dataframe.
...
Then fit a `gam` model with a combination of random effects and ICAR components: 
